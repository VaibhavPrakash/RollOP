...
func l2Deployer(backend *backends.SimulatedBackend, opts *bind.TransactOpts, deployment deployer.Constructor) (*types.Transaction, error) {
	...
	case "InterchainGasPaymaster":
		_, tx, _, err = bindings.DeployInterchainGasPaymaster(opts, backend)
	case "StorageGasOracle":
		_, tx, _, err = bindings.DeployStorageGasOracle(opts, backend)
	case "LegacyMultisigIsm":
		_, tx, _, err = bindings.DeployLegacyMultisigIsm(opts, backend)
	case "PriceFeedEth":
		_, tx, _, err = bindings.DeployPriceFeedEth(opts, backend)
	default:
